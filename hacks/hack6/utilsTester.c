...
/**
 * New test case: Test RGB to CMYK for black
 */
...
/**
 * New test case: Test RGB to CMYK for white
 */
static void testRgbToCmykWhite(void **state) {
    double c, m, y, k;
    rgbToCMYK(255, 255, 255, &c, &m, &y, &k);
    assert_true(isClose(c, 0.0) && isClose(m, 0.0) && isClose(y, 0.0) && isClose(k, 0.0));
}

/**
 * New test case: Test CMYK to RGB for random values
 */
static void testCmykToRgbRandom(void **state) {
    int r, g, b;
    cmykToRGB(0.4, 0.5, 0.6, 0.2, &r, &g, &b);
    assert_true(isCloseInt(r, 122) && isCloseInt(g, 102) && isCloseInt(b, 81));
}
...
int main(int argc, char **argv) {
    ...
    const struct CMUnitTest tests[] = {
            ...
            cmocka_unit_test(testRgbToCmykWhite),
            cmocka_unit_test(testCmykToRgbRandom),
